...
public class AABB {
    ...
    private AABB(PlayerData playerData) {
        this.min = getMin(playerData);
        this.max = getMax(playerData);
    }
    ...
    private Vector getMin(PlayerData playerData) {
        return playerData.getPlayer().getLocation().toVector().add(new Vector(-0.3, 0, -0.3));
    }

    private Vector getMax(PlayerData playerData) {
        return playerData.getPlayer().getLocation().toVector().add(new Vector(0.3, 1.8, 0.3));
    }
    ...
    public static AABB from(PlayerData playerData) {
        return new AABB(playerData);
    }
    ...
    public boolean collides(Raytrace ray, double tmin, double tmax) {
        for (int i = 0; i < 3; i++) {
