...
public class AimK
    ...
    public AimK() {
        super(CheckType.AIM_ASSISTK, "K", "AimAssist", CheckVersion.EXPERIMENTAL);
        this.violations = -1.0;
    }
    ...
    public void handle(PlayerData playerData, PlayerLocation from, PlayerLocation to, long timestamp) {
        ...
        if ((double)diffPitch > 0.0 && (double)diffPitch < 0.0094 && (double)diffPitchRate > 1.0 && (double)diffYawRate > 1.0 && (double)diffYaw > 3.0 && (double)this.lastYawRate > 1.5 && (diffPitchRatePitch > 1.0f || diffYawRateYaw > 1.0f)) {
            //VL.getInstance().handleViolation(playerData, this, "" + diffPitch + " " + diffYawRate + " " + diffYaw, false);
            VL.getInstance().handleViolation(playerData, this, "Double", false);
        }
