package fun.billy.fbi.data.gui.check.other;
...
public class BadPackets
    ...
    public BadPackets() {
        super(ChatColor.RED + "BadPackets", 18);
        int total = 0;
        ...
        for (Check.CheckType type : Check.CheckType.values()) {
            if (type.getName().contains("BadPackets") && !type.getName().contains("AutoClicker") && !type.getName().contains("KillAura")) {
    ...
    public void onClick(ClickData clickData) {
        ...
        if (slot <= this.getInventory().getSize() && slot >= 0) {
            ...
            } else if (clickData.getClickType().isLeftClick() && checkStack.getItemMeta().getDisplayName().equalsIgnoreCase(ChatColor.RED.toString() + ChatColor.BOLD.toString() + type.getName())) {
            ...
            } else if (clickData.getClickType().isRightClick() && checkStack.getItemMeta().getDisplayName().equalsIgnoreCase(ChatColor.RED.toString() + ChatColor.BOLD.toString() + type.getName())) {
                clickData.getPlayer().closeInventory();
